...
class DemoRoomSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $hcmDistrict = District::query()->where('city_id', $hcm?->id)->first();
        $hnDistrict = District::query()->where('city_id', $hn?->id)->first();
        $dnDistrict = District::query()->where('city_id', $dn?->id)->first();

        $hcmWard = Ward::query()->where('district_id', $hcmDistrict?->id)->first();
        $hnWard = Ward::query()->where('district_id', $hnDistrict?->id)->first();
        $dnWard = Ward::query()->where('district_id', $dnDistrict?->id)->first();
